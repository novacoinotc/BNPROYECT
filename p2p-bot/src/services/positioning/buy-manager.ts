  ...
  private async updateAd(ad: BuyAd): Promise<void> {
    ...
    if (assetConfig.mode === 'follow' && assetConfig.followTarget) {
      ...
      if (result?.success) {
        ...
        logger.warn(`⚠️ [BUY] ${ad.asset}: Follow fallido - target "${assetConfig.followTarget}" no encontrado, usando Smart`);
      }
    }
    ...
    if (diff < 0.01) {
      logger.debug(`✓ [BUY] ${ad.asset}: Sin cambio (actual=${ad.currentPrice.toFixed(2)}, target=${targetPrice.toFixed(2)}, diff=${diff.toFixed(4)})`);
      return;
    }
