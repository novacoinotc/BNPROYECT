...
const MIN_SURPLUS_FIAT_VALUE = 1;
...
async function testSmartBuy(asset: string) {
  console.log(`\n${'='.repeat(60)}`);
  console.log(`SMART BUY TEST: ${asset}/${FIAT}`);
  console.log(`${'='.repeat(60)}`);

  // For BUY ad, we search with tradeType=SELL to find other buyers
  // (From client perspective: "I want to sell" shows BUY ads from buyers)
  console.log(`\nSearching with tradeType=SELL (finds BUY ads from other buyers)...`);

  const ads = await searchAds(asset, FIAT, 'SELL');
  ...
  if (qualifiedAds.length > 0) {
    // Sort by price (BUY ads: highest first since we want to beat them by going higher)
    qualifiedAds.sort((a, b) => parseFloat(b.adv.price) - parseFloat(a.adv.price));

    console.log(`\n✅ QUALIFIED COMPETITORS (sorted by price, highest first):`);
    ...
    console.log(`   Strategy: ${MATCH_PRICE ? 'Match' : `Overcut by +$${(UNDERCUT_CENTS / 100).toFixed(2)}`}`);
...
async function main() {
  console.log('╔════════════════════════════════════════════════════════════╗');
  console.log('║            SMART MODE BUY - COMPREHENSIVE TEST             ║');
  console.log('╚════════════════════════════════════════════════════════════╝');
  console.log(`\nConfig:`);
