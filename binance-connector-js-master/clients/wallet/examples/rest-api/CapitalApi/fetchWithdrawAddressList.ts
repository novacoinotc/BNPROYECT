...
async function fetchWithdrawAddressList() {
    try {
        const response = await client.restAPI.fetchWithdrawAddressList();

        const rateLimits = response.rateLimits!;
        console.log('fetchWithdrawAddressList() rate limits:', rateLimits);

        const data = await response.data();
        console.log('fetchWithdrawAddressList() response:', data);
    } catch (error) {
        console.error('fetchWithdrawAddressList() error:', error);
    }
}
